...
if(isset($_POST['btn_submit']))
{
    ...
    $ins="insert into tbl_service (service_name,service_description,subcategory_id,service_rate,service_image,artist_id)
    values('".$name."','".$details."','".$subcat."','".$rate."','".$photo."','".$_SESSION['aid']."')";
    if($con->query($ins))
    {
        ?>
        <script>
...
if(isset($_GET["did"]))
{
    $del = "delete from tbl_service where service_id=".$_GET["did"];
    if($con->query($del))
    {
        ?>
        <script>
...
<body>
    ...
    <table>
        ...
        <?php
        $i=0;
        $sel="Select * from tbl_service s inner join tbl_subcategory sc on s.subcategory_id=sc.subcategory_id inner join tbl_category c on sc.category_id=c.category_id where artist_id=".$_SESSION['aid'];
        $res=$con->query($sel);
        while($row=$res->fetch_assoc())
        {
            $i++;
            ...
            <td><?php echo $row['subcategory_name'] ?>
            <td><?php echo $row['service_rate'] ?></td>
            <td><img src="../Assets/Files/Services/<?php echo $row['service_image'] ?>" width="150" alt=""></td>
